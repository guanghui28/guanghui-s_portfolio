...
function Project({ title, description, tags, imageUrl }: ProjectType) {
	...
	return (
		...
		>
			<section className=" rounded-lg bg-gray-100 max-w-[42rem] border border-black/5 overflow-hidden sm:pr-8 relative sm:h-[20rem] hover:bg-gray-200 transition dark:bg-white/10 dark:hover:bg-white/20">
				<div className="py-4  px-5 pb-7 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full sm:group-even:ml-[18rem]">
					...
					<ul className="mt-4 flex flex-wrap gap-2 sm:mt-auto">
						{tags.map((tag, index) => (
							<li
								className="bg-black/[0.7] text-white uppercase tracking-wider rounded-full px-3 py-1 text-xs dark:text-white/70"
								key={index}
							>
								{tag}
							</li>
