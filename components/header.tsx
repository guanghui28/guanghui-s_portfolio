...
const Header = () => {
	const { activeSection, updateActiveSectionWithTime } =
				...
				<ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5">
					{links.map((link) => (
						...
						>
							<Link
								className={clsx(
									"w-full flex items-center justify-center p-3 hover:text-gray-950 transition text-md dark:text-gray-500 dark:hover:text-gray-300",
									{
								...
								href={link.hash}
								onClick={() => updateActiveSectionWithTime(link.name, true)}
							>
								{link.name}
								{link.name === activeSection && (
									<motion.span
